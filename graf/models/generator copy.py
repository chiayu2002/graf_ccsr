...
class Generator(object):
    ...
    def __call__(self, z, label, rays=None, return_ccsr_output=False):
        ...
        if hasattr(self, 'estimator') and self.estimator is not None:
            render_kwargs['estimator'] = self.estimator
            render_kwargs['use_nerfacc'] = not self.use_test_kwargs
        # 優先級2: 從 render_kwargs（如果已經存在）
        elif 'estimator' in render_kwargs and render_kwargs['estimator'] is not None:
            render_kwargs['use_nerfacc'] = not self.use_test_kwargs
        # 優先級3: 禁用 NerfAcc
        else:
            render_kwargs['estimator'] = None
            render_kwargs['use_nerfacc'] = False
    
        if not hasattr(self, '_nerfacc_debug_printed'):
            print(f"\n[Generator.__call__ Debug]")
            print(f"  self.estimator exists: {hasattr(self, 'estimator')}")
            print(f"  self.estimator value: {self.estimator if hasattr(self, 'estimator') else 'N/A'}")
            print(f"  render_kwargs['estimator']: {render_kwargs.get('estimator')}")
            print(f"  render_kwargs['use_nerfacc']: {render_kwargs.get('use_nerfacc')}")
            print(f"  use_test_kwargs: {self.use_test_kwargs}")
            self._nerfacc_debug_printed = True
